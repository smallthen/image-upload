var upload:any= (function () {
  var upload: any = {},
    upImg: any= document.getElementsByClassName("upImg"),
    allImg: any= document.getElementById("allImg"),
    haveSrc:Array<any> = [];
  ...
  var closeImg = function (obj: any) {
    var thisImg:any = obj.parentNode;     //获取删除图片的父元素
  ...
  var watch = function (obj: any) {            //预览
    var imgWatch: any = document.createElement("div");
  ...
  upload.putImg = function (obj:any) {                   //点击点击上传按钮的函数
    var file: any = obj.files[0];   //获取文件引用
    var oFReader: any = new FileReader();
    ...
    oFReader.onload = function (oFREvent) { //当读取操作成功完成时调用.
      var src:number = oFREvent.target.result;     //获取src
      if (haveSrc.indexOf(src) > -1) {      //判断内部是否已存在该图片的src
        return;
      } else {
        haveSrc.push(src);             //未重复，添加入数组
      }
      var img: any = document.createElement("img");   //创建图片
      img.src = src;                       //赋值src
      img.onclick = function () {    //点击图片进行预览
        watch(img);
      }
      var close:any = document.createElement("div");   //创建删除图片按钮
      close.className = 'close';
      close.onclick = function () {      //点击删除
        closeImg(close)
      }
      var uploadImg: any = obj.parentNode;    //整个按钮的div
      var uploadButton: any = uploadImg.getElementsByClassName("uploadButton")[0];    //获取上传按钮
  ...
  upload.closeWatch = function () {    //关闭预览
    var imgWatch: any = allImg.getElementsByClassName("imgWatch")[0];
